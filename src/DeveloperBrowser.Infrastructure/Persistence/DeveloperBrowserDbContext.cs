...
public sealed class DeveloperBrowserDbContext(DbContextOptions<DeveloperBrowserDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<SavedRequestEntity>(entity => { entity.HasKey(x => x.Id); entity.Property(x => x.SecretStoreKey).IsConcurrencyToken(); entity.Property(x => x.Name).HasMaxLength(240).IsRequired(); entity.Property(x => x.Method).HasMaxLength(10).IsRequired(); entity.Property(x => x.Url).IsRequired(); entity.HasIndex(x => x.CollectionId); entity.HasIndex(x => x.FolderId); });
...
public sealed class SavedRequestEntity { public string? SecretStoreKey { get; set; } public Guid Id { get; set; } = Guid.NewGuid(); public Guid CollectionId { get; set; } public Guid? FolderId { get; set; } public required string Name { get; set; } public string? Description { get; set; } public required string Method { get; set; } public required string Url { get; set; } public string ParametersJson { get; set; } = "[]"; public string HeadersJson { get; set; } = "[]"; public string Body { get; set; } = string.Empty; public string ContentType { get; set; } = "application/json"; public string AuthType { get; set; } = "None"; public string? AuthToken { get; set; } public string? AuthUsername { get; set; } public string? AuthPassword { get; set; } public int SortOrder { get; set; } public DateTimeOffset CreatedAt { get; set; } = DateTimeOffset.UtcNow; public DateTimeOffset UpdatedAt { get; set; } = DateTimeOffset.UtcNow; }
